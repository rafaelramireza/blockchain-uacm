...
# → SS_EN_CURSO → SS_LIBERADO → TITULADO
# ================================================================================
...
echo "INSCRITO → DOC_VALIDADO → ACTIVO → CERTIFICADO → SS_EN_CURSO → SS_LIBERADO → TITULADO"
...
HASH_SS_INICIO=$(echo -n "${MATRICULA}_FOL-2026-SS-INICIO" | sha256sum | awk '{print $1}')
HASH_SS_LIBERADO=$(echo -n "${MATRICULA}_FOL-2026-SS-LIBERADO" | sha256sum | awk '{print $1}')
...
peer chaincode invoke $ORDERER_ARGS $CHANNEL_ARGS $PEERS_ARGS \
...
peer chaincode invoke $ORDERER_ARGS $CHANNEL_ARGS $PEERS_ARGS \
    -c "{\"Args\":[\"IniciarServicioSocial\",\"$MATRICULA\",\"$HASH_SS_INICIO\"]}"
sleep 3

echo "CU-05 Registrar liberación del Servicio Social"
peer chaincode invoke $ORDERER_ARGS $CHANNEL_ARGS $PEERS_ARGS \
    -c "{\"Args\":[\"LiberarServicioSocial\",\"$MATRICULA\",\"$HASH_SS_LIBERADO\"]}"
sleep 3
...
peer chaincode query $CHANNEL_ARGS \
